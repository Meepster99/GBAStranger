...
public: 
	...
	__attribute__((noinline, target("arm"), section(".iwram"))) void draw(u8 (&collisionMap)[14][9], FloorTile* (&floorMap)[14][9]);
...
public:
	...
	__attribute__((noinline, section(".iwram"))) void loadTiles(u8* floorPointer, SecretHolder* secrets, int secretsCount, const char* exitDest);
	...
	//#define TILEMANAGERATTRIBUTES __attribute__((section(".ewram")))
	#define TILEMANAGERATTRIBUTES
	
	TILEMANAGERATTRIBUTES bool exitRoom(); // just in here as a temporary measure
	TILEMANAGERATTRIBUTES bool enterRoom(); // just in here as a temporary measure
	
	//bool needUpdate = false;
	TILEMANAGERATTRIBUTES void doVBlank();
};
